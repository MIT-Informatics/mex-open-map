...
seccion_base.sf2 <- read_sf("C:/Users/Brandon/Documents/Strategy_Paper_SHP/2017_11_test/11/DISTRITO.shp", check_ring_dir=TRUE)
```
...
```{r}
tempPlan <- planProcessing.df %>% filter(edon == 7 | edon == 11)
```
...
seccion_base.sf2 %>% filter(entidad==tempPlan[[i,"edon"]]) %>% mutate(district = distrito) %>%
  left_join(tempPlan[[i,"plan"]][[1]],by=c("district")) %>% mutate(district=factor(district)) %>%
  ggplot() + aes(fill=district) + geom_sf()
i<-26
