...
winvotes.df <- votes.2015.df %>% 
  mutate(edon=factor(edon,labels = statecodes.df %>% pull(tla))) %>%
  group_by(edon,disn_adopt,actor) %>% 
  summarize(actor_vote=sum(votes,na.rm=FALSE)) %>%
  group_by(edon,disn_adopt) %>%
  summarize(distmaxvote=max(actor_vote),distwinvote=sort(actor_vote,decreasing=TRUE)[2])

geom_boxplot(
  data = winvotes.df, 
  aes(x=edon,y=distwinvote,color="Winning Vote Threshold") ) -> p1.1

plot(p1 + p1.1 + coord_flip())
...
majProp.tib <- proptrans.df %>% filter(VALIDPLAN==TRUE, actortype=="major") %>% select(-plan)
majProp.tib 
```
