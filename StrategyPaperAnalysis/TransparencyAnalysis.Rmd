...
thirdparty.tab %>% 
  ...
  tab_source_note(
    source_note = "Note: Does not INE interventions to modify plans during decision phase." ) %>%
...
contestSum.df %>%
  ...
  summarize( 
    ...
    algorithm_lost = sum(algorithm_lost)/n(),
 ...
 gt() %>%
    ...
    tab_header(title="Unexpected Winners",
               subtitle="Percentage of all states, won in an unexpected way.") %>%
    tab_source_note( source_note = "Note: Column totals do not sum up to 100% because each state can be unexpected in multiple categories." ) %>%
    data_color (
      columns=2:5, 
      colors = scales::col_numeric( 
      palette = c( "white","red"), 
...
propmatch.df %>% 
  filter(actortype %in% c("major","minor")) %>%
  mutate(actortype = fct_drop(actortype)) %>%
  select( "actortype", "competitive_difference", "proposer_win_difference", "proposer_seat_difference","proposer_competitive_margin_difference", "proposer_composite_difference") %>%
  relocate("proposer_seat_difference","competitive_difference","proposer_win_difference","proposer_competitive_margin_difference","proposer_composite_difference") %>% 
  tbl_summary(by=actortype,
              type = list(everything() ~ "continuous"),
          statistic = list(all_continuous() ~ "{mean} ({sd}) [{min}-{max}]"),
          missing="no",
          label = list("proposer_seat_difference" ~ "Proposer Seat Change","competitive_difference" ~ "Competitive Seat Change","proposer_win_difference"~ "Proposer Win Margin Change","proposer_competitive_margin_difference"~"Proposer Competive Margin Change" ,"proposer_composite_difference" ~ "Total Proposer Score Change")
          ) %>%
 as_gt() %>% 
