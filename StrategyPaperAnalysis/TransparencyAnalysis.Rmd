...
comp.tbl %>% group_by(year,creator_coded) %>% filter(creator_coded == "PAN" | creator_coded == "PRI" | creator_coded == "PRD" | creator_coded == "PVEM" | creator_coded == "Coalition") %>% filter(year == 2013) %>% gt()
comp.tbl %>% group_by(year,creator_coded) %>% filter(creator_coded == "PAN" | creator_coded == "PRI" | creator_coded == "PRD" | creator_coded == "PVEM" | creator_coded == "Coalition") %>% filter(year == 2017) %>% gt()
...
basediffs.tbl %>% 
  ...
  labs(title="Change in Seccion Assignment: Proposals vs. 2004 Baseline",
       caption = "(Mean  proposed plans in edon & year.)", y="Proportion Changed", x="Edon", color="apportionment change")
...
validPlans.tbl %>% filter(win_final) %>% 
  ggplot(aes(x=edon,y=alg_diff_percent,color=creator_label)) +
  geom_point() + 
  geom_segment( aes(x=edon, xend=edon, y=0, yend=alg_diff_percent)) + 
  facet_wrap(vars(year)) + 
     geom_ysidedensity(aes(x=stat(density),color="(Total)")) +
 labs(title="Change in Seccion Assignment: Winning Plan vs. Algorithmic proposal",
       caption = "", y="Proportion Changed", x="Edon", color="creator")
