...
stratpos.tbl <- ys.proposed %>% group_by(actor, StratPos) %>% summarise("% Proposed"= round(sum(didPropose)/n()*100, digits = 1))
stratpos.tbl <- stratpos.tbl %>% as_tibble() %>% arrange(StratPos) %>% pivot_wider(., id_cols = actor, names_from = StratPos, values_from = `% Proposed`)
stratpos.tbl$`1`[is.na(stratpos.tbl$`1`)] <- "-"
stratpos.tbl$`2`[is.na(stratpos.tbl$`2`)] <- "-"
stratpos.tbl$`3`[is.na(stratpos.tbl$`3`)] <- "-"
stratpos.tbl$`4`[is.na(stratpos.tbl$`4`)] <- "-"
stratpos.tbl$`5`[is.na(stratpos.tbl$`5`)] <- "-"
stratpos.tbl$`6`[is.na(stratpos.tbl$`6`)] <- "-"
...
model.results <- ys.proposed %>% filter(year == 2017)%>% clmm(as.ordered(didPropose)~as.ordered(StratPos)+(1|actor),., Hess = TRUE)
...
Number of Proposals
```{r}
clm.mod <- ys.proposed %>% clm(as.factor(numproposals)~as.ordered(PartyStratPos), data=., HESS=TRUE)
summary(clm.mod)

mean(ys.proposed$numproposals)
```
...
Vraw.2015 %<>% pivot_longer(., cols = c("pan", "pri", "prd", "pvem", "pt", "mc", "pna", "morena", "es"),
                            names_to = "actor",
                            values_to = "votes_2015")
Vraw.2015 <- Vraw.2015 %>%
  mutate(actor = toupper(actor)) %>%
  select(edon, efec, actor, votes_2015) %>%
  na.omit() %>%
  group_by(edon, actor) %>%
  summarize(actor_votes = sum(votes_2015), total_votes = sum(efec))

#Merge w/ ys.proposed by state by party

vote_share_mod.df <- left_join(ys.proposed, Vraw.2015, by=c("edon", "actor"))
vote_share_mod.df <- vote_share_mod.df %>%
  filter(year == 2017) %>%
  rowwise() %>%
  mutate(vote_share = (actor_votes/total_votes)*100)

vote_share_mod <- glm(didPropose ~ vote_share, data = vote_share_mod.df, binomial)
summary(vote_share_mod)
vote_share_mod <- lm(numproposals ~ vote_share, data = vote_share_mod.df)
summary(vote_share_mod)
```
...
```
#Graph
#Compares Alogrithm Proposals to Winning Proposals
#They track closely to 1:1
```{r}

algo1 <- propfull.df %>%
  filter(stage == 1) %>%
  mutate(Score_1 = case_when(stage == 1 ~ SCORE))
algo3 <- propfull.df %>%
  filter(stage == 3) %>%
  mutate(Score_3 = case_when(stage == 3 ~ SCORE))
algo5 <- propfull.df %>%
  filter(stage == 5) %>%
  mutate(Score_5 = case_when(stage == 5 ~ SCORE))

algo <- left_join(algo1, algo5, by = c("edon", "year"))
algo <- left_join(algo, algo3, by = c("edon", "year"))
algo <- algo %>%
  mutate(EQ_1 = if_else(Score_1 == Score_5, 1, 0),
         EQ_2 = if_else(Score_3 == Score_5, 1, 0))

algo %>% ggplot(aes(x = Score_1, y = Score_5, color = EQ_1)) +
  geom_jitter() + facet_wrap(~year)
algo %>% ggplot(aes(x = Score_3, y = Score_5, color = EQ_2)) +
  geom_jitter()
...
```{r}
algo_1.1 <- algo1 %>%
  select(edon, year, Score_1) %>%
  rename(WinSCORE_1 = Score_1)
algo_3.1 <- algo3 %>%
  select(edon, year, Score_3) %>%
  rename(WinSCORE_2 = Score_3)
algo_5.1 <- algo5 %>%
  select(edon, year, Score_5) %>%
  rename(WinSCORE_final = Score_5)

diff <- left_join(propfull.df, algo_1.1, by = c("edon", "year"))
diff <- left_join(diff, algo_3.1)
diff <- left_join(diff, algo_5.1)

diff <- diff %>%
  filter(PROPOSED == TRUE) %>%
  filter(stage == 2 | stage == 4) %>%
  group_by(edon, year, stage) %>%
  mutate(diff_1 = (SCORE - WinSCORE_1),
         diff_2 = (SCORE - WinSCORE_2),
         diff_3 = (SCORE - WinSCORE_final))

diff %>%
  ggplot(aes(stage, diff_1)) + geom_jitter() + facet_wrap(~year)
#Partisan scores in stage 1 & 2 relative to the initial algorithm proposal (stage == 1)
diff %>%
  ggplot(aes(stage, diff_2)) + geom_jitter() + facet_wrap(~year)
#Partisan scores in stage 1 & 2 relative to the score which "won" the  first round (stage ==3)
diff %>%
  ggplot(aes(stage, diff_3)) + geom_jitter() + facet_wrap(~year)
#Partisan scores in stage 1 & 2 relative to the score which won the redistricting process (in each state) (stage == 5)
```
...
WM_coal.df <- left_join(temp2, temp1, by = c("edon","year"))
##Create dummy matching minimum scores to the winning score
WM_coal.df %<>% group_by(edon, year, govlevel) %>% 
  select(edon, year, actor, actortype.x, FS_SCORE, SS_SCORE, partisan_FS_SCORE,
         partisan_SS_SCORE, Win_SCORE, CoalitionStratPos.x, CoalitionType.x) %>%
  mutate(ACTOR_WIN1 = if_else(partisan_FS_SCORE == Win_SCORE, 1, 0),
         ACTOR_WIN2 = if_else(partisan_SS_SCORE == Win_SCORE, 1, 0),
