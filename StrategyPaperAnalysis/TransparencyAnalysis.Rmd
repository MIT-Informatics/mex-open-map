...
contestSum.df %<>% rowwise() %>% mutate(lowestvalid = min(phase1_start,phase2_plans,phase3_win,phase4_plans,final_win),phase2_lowest = min(phase1_start,phase2_plans),final_lowest=min(phase3_win,phase4_plans,final_win),best_lost=final_win!=lowestvalid)
...
#planProp_old.df <- proptrans.df %>% filter(!is.na(planid)) %>% group_by(planid) %>% summarize(stages=list(sort(unique(stage))),rscore=unique(rscore),edon=unique(edon),year=unique(year),VALIDPLAN=unique(VALIDPLAN),tla=unique(tla), hasplan=unique(hasplan))

planProp.df <-
  proptrans.df %>% 
  filter(!is.na(planid)) %>% 
  group_by(planid) %>% 
  mutate(stages=list(sort(unique(stage)))) %>%
  select(stages,rscore,edon,year,VALIDPLAN,tla,hasplan) %>%
  arrange(hasplan,VALIDPLAN,.by_group=TRUE) %>%
  slice_tail(n=1)
...
planProp.df %<>% 
   group_by(year,edon) %>% 
   mutate(best_nonmissing_score=min(rscore[hasplan==TRUE])) %>% 
...
saveRDS(propfull.df,"propfull.RDS")
saveRDS(votes.2012.df,"votes.2012.RDS")
saveRDS(votes.2015.df,"votes.2015.RDS")
saveRDS(votes.2018.df,"votes.2018.RDS")
saveRDS(statecodes.df,"statecodes.RDS")
```

#Transparency Analysis
```{r load-checkpoint-trans}
rm(list=ls())
...
propfull.df <- readRDS("propfull.RDS")
source("gt_theme_538.R")
```
...
actors.df <- readRDS("actors.RDS")
propfull.df <- readRDS("propfull.RDS")
votes.2015.df <- readRDS("votes.2015.RDS")
votes.2018.df <- readRDS("votes.2018.RDS")
votes.2012.df <- readRDS("votes.2012.RDS")
```

```{r}
source("PrepSeatScores.R",verbose=FALSE)
saveRDS(planProcessing.df,"planprocessing.RDS")
```
...
votes.2018.df <- readRDS("votes.2018.RDS")
#votes.2012.df <- readRDS("votes.2012.RDS")
planProcessing.df <- readRDS("planprocessing.RDS")
statecodes.df <- readRDS("statecodes.RDS")
source("gt_theme_538.R")
source("plandiff.R")
```
...
planFull.df %<>% left_join(by="planid", proptrans.df %>% group_by(planid) %>% slice_head(n=1) %>% select(starts_with("off_")) )
...
comp.tbl %<>% bind_rows(
  planProcessing.df %>% filter(str_starts(planid,"base")) %>% 
    mutate(year=2004) %>%
    mutate(plan_2018_actors_winMargins_compCount=plan_2018_actors_winMargins_compCount2, 
                        plan_2015_actors_winMargins_compCount=plan_2015_actors_winMargins_compCount2,
                        plan_2012_actors_winMargins_compCount=plan_2012_actors_winMargins_compCount2) %>%
   group_by(year) %>% 
...
validScores.tbl <- validPlans.tbl %>%
  ...
  rowwise() %>%
  mutate(
    measured_splits = ifelse(year==2013, plan_2012_muniSplits, plan_2015_muniSplits ),
    ...
    off_splits = off_splits,
    off_indig = off_indig,
    off_compact = off_compact,
    municiple_splits = ifelse(is.na(off_splits),measured_splits,off_splits)
  ) %>% 
  select ("planid", "tla", "municiple_splits", "competitive_districts", 
          "worst_population_deviation", "average_winning_margins", "win_margins","official_score", starts_with("off_"), starts_with("diff_"), contains("lits")) %>% 
  relocate("municiple_splits","off_splits")
...
# diagnostic plot to compare different split calculationsV
( validScores.tbl %>% ggplot(aes(x=off_splits,y=plan_2015_muniSplits)) + geom_point(alpha=.1) ) /
  ( validScores.tbl %>% ggplot(aes(x=off_splits,y=plan_2012_muniSplits)) + geom_point(alpha=.1) ) /
    ( validScores.tbl %>% ggplot(aes(x=off_splits,y=plan_2018_muniSplits)) + geom_point(alpha=.1) )

# look at extreme difference
planProcessing.df %>% filter(planid=="6.29757-21-2017") %>% select(plan_2015) -> splitcmp.df
splitcmp.df <- splitcmp.df[[1,1]][[1]]
...
counterScore <- function(p1,p2,proposer) {
  ...
  rows.tib <- left_join(by="planid",tibble(planid=c(p1,p2)), validScores.tbl) %>%
    ...
    mutate(
      proposer_win_margin = ifelse( is.null(win_margins) , NA_real_ , 
                                    (win_margins %>% 
                                       filter(win_actor==proposer) %>%
                                       pull(win_margin) %>%
                                       mean(na.rm=TRUE) )
      ),
      proposer_win_spread = ifelse( is.null(win_margins) , NA_real_ , 
                                    (win_margins %>% 
                                       filter(win_actor==proposer) %>%
                                       pull(win_margin) %>%
                                       spreadn() )
      ),
      proposer_win_margin_capped = ifelse( is.null(win_margins) , NA_real_ , 
                   (win_margins %>% 
                      filter(win_actor==proposer) %>% 
                      pull(win_margin) %>% 
                      pmin(.1) %>%
    ...
    comp_margin = ifelse( is.null(win_margins) , NA_real_ , 
            (win_margins %>% filter(win_margin<.02)  %>% 
               pull(win_margin) %>% mean(na.rm=TRUE) %>% 
               replace_na(0))
      ),
     pcomp_margin = ifelse( is.null(win_margins) , NA_real_ , 
            (win_margins %>% filter(win_margin<.02)  %>% 
               mutate( isp = win_actor==proposer) %>%
               mutate( pwin_margin=ifelse(isp,win_margin,-1*win_margin)) %>%
               pull(pwin_margin) %>% mean(na.rm=TRUE) %>% 
               replace_na(0))
      ),
      proposer_packed_margin = ifelse( is.null(win_margins) , NA_real_ , 
                                    (win_margins %>% 
                                       filter(win_actor==proposer, win_margin > .1) %>%
                                       pull(win_margin) %>%
                                       mean(na.rm=TRUE) )
      ),
    ) %>%
    mutate(proposer_win_margin=replace_na(proposer_win_margin,0)) %>%
    mutate(proposer_packed_margin=replace_na(proposer_packed_margin,0)) %>%
    mutate(proposer_win_spread=replace_na(proposer_win_spread,0)) %>%
  ...
  diff_cols %<>%   rename(score_difference=official_score, 
                          ...
                          proposer_competitive_margin_difference= pcomp_margin,
                          proposer_win_difference=proposer_win_margin,
                          proposer_win_spread_difference=proposer_win_spread,
                          proposer_packed_difference=proposer_packed_margin,
                          proposer_win_difference_capped=proposer_win_margin_capped,
                          proposer_seat_difference = proposer_win_seat,
                          compactness_difference =off_compact) %>%
    mutate(proposer_composite_difference = 1000 * proposer_seat_difference + 100 * competitive_difference +   +10* proposer_win_difference_capped + proposer_competitive_margin_difference ) %>%
...
propmatch.df %>% 
  ...
  select("score_difference","split_difference", "competitive_difference","proposer_competitive_margin_difference", "population_difference", "win_difference","actortype", "proposer_win_difference", "proposer_seat_difference","competitive_margin_difference", "proposer_composite_difference",
...
my_tbl_stats <- function(data, variable, ..., rnd_digits=4) {
  vec <- data[[variable]] 
  v.mean <- mean(vec,na.rm=TRUE)
  v.sd <-  sd(vec,na.rm=TRUE)
  v.n <- length(na.omit(vec))
  v.sem <- v.sd/sqrt(v.n)
  #v.sig095 <- ifelse( (round(v.mean, digits=rnd_digits)!=0) & ((v.mean - 1.96 * v.sem)) & (0 <= (v.mean+1.96*v.sem)),"","*")
    v.sig095 <- ifelse( (round(v.mean, digits=rnd_digits)!=0) && (0 < (v.mean - 1.96 * v.sem) | 0 > (v.mean+1.96*v.sem)),"*","")

  v.min <- min(vec,na.rm=TRUE)
  v.max <- max(vec,na.rm=TRUE)
  dplyr::tibble(
    mean = v.mean,
    sd =  v.sd,
    sem = v.sem,
...
my_style_sigfig<-function(x,digits=4,...) {
  if (is.numeric(x)) {
...
propmatch.df %>% 
  filter(actortype %in% c("major","minor")) %>%
  mutate(actortype = fct_drop(actortype)) %>%
  select( "actortype", "competitive_difference", "proposer_win_difference", "proposer_seat_difference","proposer_competitive_margin_difference", "proposer_win_spread_difference",
          "proposer_packed_difference",
          "proposer_composite_difference") %>%
  relocate("proposer_seat_difference","competitive_difference","proposer_win_difference","proposer_win_spread_difference", "proposer_competitive_margin_difference","proposer_packed_difference", "proposer_composite_difference") %>% 
  tbl_custom_summary(by=actortype,
              type = list(everything() ~ "continuous"),
          stat_fns = all_continuous() ~ my_tbl_stats,
          #statistic = list(all_continuous() ~ "{mean} ({sd}) [{min} ... {max}]"),
          statistic = list(all_continuous() ~ "{mean} {sig095} ({sem}) [{min} ... {max}]"),
          missing="no",
           digits = all_continuous() ~ my_style_sigfig,
          label = list("proposer_seat_difference" ~ "Proposer Seat Increase ","competitive_difference" ~ "Competitive Seat Increase","proposer_win_difference"~ " Margin change in winning districts for proposer","proposer_competitive_margin_difference"~"Margin change for proposer in competitive districts" ,"proposer_composite_difference" ~ "Total Composite Score Change",
                       "proposer_packed_difference" ~ "Margin change in districts packed for proposer", "proposer_win_spread_difference"~ " WIn margin increase within winning districts")
          ) %>%
  modify_footnote(all_stat_cols() ~ "mean  (standard error) [min ... max] ; '*' = significance at the .95 level") %>%
 as_gt() %>% 
  tab_header(title="Counterproposal Differences by Actor Type - Theoretical Predictors")
...
#votes.2015.df <- read_csv("mxDistritos-data/raw-seccion-2015.csv",              col_types=cols( .default = col_double()))
#votes.2018.df <- read_csv("mxDistritos-data/raw-seccion-2018.csv", col_types=cols( .default = col_double()))
#votes.2018.df %<>%  clean_vraw()
#votes.2015.df %<>%  clean_vraw()
